...
function Sequence(sequence,seqid,isoformName) {
    var isoName;
    if (isoformName !== undefined) isoName = isoformName;
    else isoName = "";
    var seqName;
    if (seqid !== undefined) seqName = seqid;
    ...
    function renderHtml(divId,options) {
	    ...
	    lineJump = options.charsPerLine;
	}
	
	sequenceHeader = 'sequenceHeader_' + options.id;
	inputSearchSeq = 'inputSearchSeq_' + options.id;
	sequenceBody = 'sequenceBody_' + options.id;
	scroller = 'scroller_' + options.id;
	charNumbers = 'charNumbers_' + options.id;
	fastaSeq = 'fastaSeq_' + options.id;
	coverageLegend = 'coverageLegend_' + options.id;
	sequenceToolbar = 'sequenceToolbar_' + options.id;
	CPLChoice = 'CPLChoice_' + options.id;
	peptideHighlighted = 'peptideHighlighted_' + options.id;
	stringSelected = 'stringSelected_' + options.id;
	
        var sources = "<div id=" + sequenceHeader +" class=\"row\" style=\"border-bottom: 1px solid #E7EAEC;padding-bottom:5px;margin:0px 0px 15px\"><div>" +	    
            "<h3 style=\"color:#DF0101;display:inline-block;vertical-align:middle;\">" + seqName + "</h3><br />" +

	    "<span style=\"color:#084B8A;padding:8px 5px;background-color:white;margin-right:10px;vertical-align:middle;\">Location:" + options.location + "</span>" +
	    
            "<span style=\"color:#298A08;padding:8px 5px;background-color:white;margin-right:10px;vertical-align:middle;\">Length: {{sequenceLength}} bp</span>";

	if ( options.flank > 0 ){
	    sources += "<span style=\"color:#DF7401;padding:8px 5px;background-color:white;margin-right:10px;vertical-align:middle;\">Flank Length:" + options.flank + "bp</span>";
	}

	if( options.revComp === true ){
	    sources += "<span style=\"color:#a05ce0;padding:8px 5px;background-color:white;margin-right:10px;vertical-align:middle;\">ReverseComplemented</span>";
	}
    ...
    function addSequenceSearch() {
        $(divID + ' #' + sequenceHeader).append('<div><input id='+ inputSearchSeq +' type=\"text\" class=\"form-control pull-left\" style=\"width:40%;margin-top:3px;\" placeholder=\"Search in sequence.. (Regex supported)\"></div>');
        sequenceSearch();
